...
def process_customer_files_pass3(customer_path_p1, logger):
    """
    ...
    try:
        ...
        for file in os.listdir(TAXONOMY_DIR):
            if file.endswith(".json"):
                ...
                logger.info(f"Starter validering for {base_name}.csv i {customer_path_p1}")

                errors = validate_csv_against_schema(csv_path, json_def, labels, can_be_empty)

                if errors:
                    logger.error(f"Feil ved validering av {csv_path}:")
                    for r in errors:
                        logger.error(f"  {r}")
                else:
                    logger.info(f"Validering for {csv_path} fullført uten feil.")

...
def fix_05_02(input_file_path, alogger, clogger=None):
    try:
        ...
        clogger.info(f"B_05.02: Filen er renset og lagret som: {temp_output_file_path}")

        # Slett original fil etter rensing
        os.remove(input_file_path)
        clogger.info(f"B_05.02: Originalfilen {input_file_path} er slettet.")

        # Gi den rensede filen originalt navn
        os.rename(temp_output_file_path, input_file_path)
        clogger.info(f"B_05.02: Renset fil er omdøpt tilbake til {input_file_path}")
    ...
    try:
        ...
        for index, row in df_b0502.iterrows():
            if row['c0060'] in df_b0501['c0010'].values:
                ...
                clogger.info(f"B_05.02_pass2: Rad {index} - c0060 '{row['c0060']}' finnes ikke i c0010 i b_05.01, sletter raden.")
        ...
        clogger.info(f"B_05.02_pass2: Originalfilen {input_file_b0502} er slettet.")

        # Gi den rensede b_05.02 filen originalt navn
        os.rename(temp_output_file_b0502, input_file_b0502)
        clogger.info(f"B_05.02_pass2: Renset b_05.02 fil er omdøpt tilbake til {input_file_b0502}")

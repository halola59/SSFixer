...
def fix_04_01(input_file_path, logger, clogger=None):
    try:
        # Les inn CSV-filen
        ...
        for index, row in rows_with_empty_c0020.iterrows():
            clogger.info(f"B_04.01: Rad {index} - c0020 er tom, sletter raden.")
        df = df[df['c0020'].notna() & (df['c0020'] != '')]

        # Fjern alle duplikatrader
        duplicate_rows = df[df.duplicated(keep=False)]
        for index, row in duplicate_rows.iterrows():
            clogger.info(f"B_04.01: Rad {index} - duplikatrad, sletter duplikatet.")
        ...
        clogger.info(f"B_04.01: Filen er renset og lagret som: {temp_output_file_path}")

        # Slett original fil etter rensing
        os.remove(input_file_path)
        clogger.info(f"B_04.01: Originalfilen {input_file_path} er slettet.")

        # Gi den rensede filen originalt navn
        os.rename(temp_output_file_path, input_file_path)
        clogger.info(f"B_04.01: Renset fil er omdøpt tilbake til {input_file_path}")

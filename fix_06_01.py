...
def fix_06_01(input_file_path, alogger, clogger=None):
    try:
        ...
        for index, row in rows_with_spaces_in_c0010.iterrows():
            CLOG(clogger, "B_06.01", index, f"c0010 '{row['c0010']}' har spaces", "Fjerner leading/trailing spaces")
        ...
        if c0040_values.size > 0:
            ...
            for index, row in rows_with_empty_c0040.iterrows():
                CLOG(clogger, "B_06.01", index, f"c0040 er tom", f"Setter verdi til '{fill_value}'")
            df['c0040'] = df['c0040'].fillna(fill_value)
        ...
        if not duplicates.empty:
            ...
            for index, row in duplicates.iterrows():
                CLOG(clogger, "B_06.01", index, f"Duplikat av kombinasjonen c0010 ('{row['c0010']}') + c0040", "Sletter raden, beholder kun en forekomst")   
